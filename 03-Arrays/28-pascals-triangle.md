...
- **Why it's not good enough**: Computing $nCr$ from scratch for every position takes $\mathcal{O}(n^3)$ operations.
...
### Idea
Multiplicative Combinatorics Row Generation: For row $i$ ($0$-indexed), start with `val = 1`. For column $j$ from 1 to $i$, compute next element as `val = val * (i - j + 1) / j` in $\mathcal{O}(1)$ time per cell.
...
- **Space Complexity**: O(1)
- **Why this is optimal**: Computes the exact $n(n+1)/2$ elements in optimal $\mathcal{O}(n^2)$ time with $\mathcal{O}(1)$ auxiliary space.
...
|---|---|---|
| `Row 0` | i=0 -> [1] | triangle=[[1]] |
| `Row 1` | i=1 -> [1, 1] | triangle=[[1],[1,1]] |
| `Row 2` | i=2 -> val=1, val=1*(2-1+1)/1=2 -> [1, 2, 1] | triangle=[[1],[1,1],[1,2,1]] |
| `Row 3` | i=3 -> val=1, val=1*3/1=3, val=3*2/2=3 -> [1, 3, 3, 1] | Final: 4 rows ✅ |

## 7. Edge Cases & Common Bugs

### Edge Cases
- numRows = 1 (`[[1]]`).
- numRows = 30 -> 64-bit `long long` intermediate math prevents overflow before division.

### Common Bugs to Avoid
- Multiplying and dividing in wrong order: must multiply `val * (i - j + 1)` before dividing by `j`.
